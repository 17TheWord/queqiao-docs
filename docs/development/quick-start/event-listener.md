...
监听已有事件，并组合为工具包中提供的结构，最后发送事件。

- 判断事件是否被取消、配置项中是否启用了该事件的监听。
- 获取玩家信息，组合为工具包中的玩家模型 `PlayerModel`。
- 根据不同事件，组合为工具包中的事件模型，如 `PlayerChatEvent`、`PlayerJoinEvent` 等。
- 使用 `GlobalContext.sendEvent(event)` 发送事件。
